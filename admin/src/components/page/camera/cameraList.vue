<template>
    <div class="product-list">
        <div class="head">
            ...
            <el-button type="primary" @click="syncCamera">同步设备</el-button>
        </div>
        <div class="content">
             <el-table
                :data="lists"
        ...
        <div class="add-camera" v-if="editCamera">
            <ReNameCamera @add="add" :originData="curCamera.deviceName"></ReNameCamera>
        </div>
    </div>
</template>
<script>
...
import ReNameCamera from './reNameCamera';
...
export default {
    ...
    methods:{
        ...
        syncCamera(){
            this.loading = true;
             this.$axios.get('/bindDeviceData ').then((res) =>{
                if(res.status === 200){
                    if(res.data.code > 0){
                        this._getLists(1);
        ...
        add(deviceName){
            ...
            this._submitForm(this.curCamera.deviceId,deviceName)
        },
        ...mapMutations({
        ...
        _submitForm(deviceId,deviceName){
            ...
            //上传请求
            me.$axios.post('/setDeviceName', formData,{
                headers:{
