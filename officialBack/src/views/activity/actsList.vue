			...
			<el-table-column  label="图片" >
                <template scope="scope" >
                    <div style="text-align:center">
                        <img :src="DefaultImageIp + scope.row.image.src" alt="" style="width:100px;">
                    </div>
	...
	export default {
		...
		methods: {
			...
			getActivityList(pageNumber) {
				...
				this.$axios.get('getActivityList',{
					params:{
			...
			handleEdit: function (index, row) {
				this.$router.push({path: '/addAct',query:{id: row.id}})
			},
			//显示新增界面
			handleAdd: function () {
